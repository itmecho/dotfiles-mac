    ...
    generic_picker({
        ...
        mappings = function(prompt_bufnr, map)
            map("i", "<cr>", function()
                ...
                neoterm.run("orca variant update " ..
                                state.get_selected_entry().value)
            end)
            map("i", "<C-l>", function()
                actions.close(prompt_bufnr)
                neoterm.run("orca variant logs " ..
                                state.get_selected_entry().value)
            end)
            return true
        end
    ...
    tb.find_files({
        ...
        attach_mappings = function(prompt_bufnr, map)
            map("i", "<cr>", function(bufnr)
                local path = "~/src/CloudExperiments/" ..
                                 state.get_selected_entry(bufnr).value
                vim.cmd("cd " .. path)
                actions.close(prompt_bufnr)
        ...
        attach_mappings = function(_, map)
            map("i", "<c-d>", function(_)
                ...
                if pcall(vim.api.nvim_buf_delete, selection.bufnr, {}) then
                    print(string.format("Deleted buffer %d: %s",
                                        selection.bufnr, selection.filename))
                    -- Delete buffer here
                end
            end)
            return true
